...
if (preg_match('/^[a-f0-9]{64}$/', $token)) {
    try {
        ...
        if ($pdo instanceof PDO) {
            $select = 'SELECT participant_code, full_name, position, organization, participation_format, qr_token, online_token FROM participants WHERE registration_status = "confirmed" AND %s = :token LIMIT 1';
