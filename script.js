...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (isAboutPage) {
        ...
        // Модули только для страницы регистрации
        initCustomSelects();
    ...
    photos.forEach((photoPath, index) => {
        ...
        slideElement.innerHTML = `
            <img src="${photoPath}" 
                 ...
                 onerror="console.error('❌ Ошибка загрузки фото ${index + 1}:', this.src); this.style.display='none';">
            <div class="photo-placeholder" style="${index === 0 ? '' : 'display: none;'}">
                <span class="photo-icon">📸</span>
                <p>Фото ${index + 1} с конференции</p>
            </div>
        `;
...
function initYandexMap() {
    ...
    console.log('🗺️ Инициализация Яндекс.Карт...');
    ...
    ymaps.ready(() => {
        try {
            console.log('✅ API Яндекс.Карт готово');
...
// ==================== АККОРДЕОН ПРОГРАММЫ (ТОЛЬКО ГЛАВНАЯ) ====================
