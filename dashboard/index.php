...
try {
    ...
    $peopleStmt = $pdo->prepare("SELECT participant_code, full_name, position, organization, email, participation_format, registration_status, registration_source, created_at, check_in_at, online_watch_seconds
      FROM participants
      WHERE event_id = :event
        AND organization <> :test_org
        AND LOWER(TRIM(organization)) NOT IN ('ovan','oivan')
...
<tr data-format="<?= h($p['participation_format']) ?>" data-status="<?= h($p['registration_status']) ?>" data-search="<?= h(mb_strtolower($p['full_name'].' '.$p['organization'].' '.$p['position'].' '.$p['email'].' '.$p['participant_code'])) ?>"><td><strong><?= h($p['full_name']) ?></strong><div class="muted"><?= h($p['participant_code']) ?></div></td><td><strong><?= h($p['organization']) ?></strong><div class="muted"><?= h($p['position']) ?></div></td><td><a href="mailto:<?= h($p['email']) ?>" style="color:#214f3b;text-decoration:none;white-space:nowrap"><?= h($p['email']) ?></a></td><td><span class="tag"><?= $p['participation_format']==='offline'?'Очно':'Онлайн' ?></span></td><td><span class="tag"><?= h(registrationSourceLabel((string)$p['registration_source'])) ?></span></td><td><span class="tag <?= $p['registration_status']==='waitlist'?'wait':($p['registration_status']==='cancelled'?'cancel':'') ?>"><?= $p['registration_status']==='confirmed'?'Подтверждено':($p['registration_status']==='waitlist'?'Ожидание':'Отменено') ?></span></td><td><?= h(date('d.m.Y H:i', strtotime($p['created_at']))) ?></td><td><?= h($present) ?></td></tr>
<?php endforeach; ?>
