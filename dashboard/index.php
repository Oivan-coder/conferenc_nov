...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
        ...
        if ($action === 'delete_test') {
            ...
            $pdo->beginTransaction();
            $testStmt = $pdo->prepare('SELECT id, participant_code, full_name, organization FROM participants WHERE id = :id AND event_id = :event AND (organization = :test_org OR LOWER(TRIM(organization)) = "ovan") LIMIT 1 FOR UPDATE');
    ...
    $testRecordsStmt = $pdo->prepare("SELECT id, participant_code, full_name, organization, participation_format, created_at, check_in_at, online_watch_seconds
      FROM participants
      WHERE event_id = :event AND (organization = :test_org OR LOWER(TRIM(organization)) = 'ovan')
      ORDER BY created_at DESC, id DESC
      LIMIT 100");
...
<section class="panel test-panel" id="test-records"><div class="panel-head"><div><h2>Тестовые записи</h2><p class="muted">Не входят в показатели. Здесь также появится запись с организацией «ovan», если она действительно хранится в БД.</p></div><span class="pill"><?= count($testRecords) ?> записей</span></div>
<?php if (!$testRecords): ?><div class="empty-note">Тестовых записей в базе нет. Подсказка «ovan» тогда сохранена локально браузером.</div><?php else: ?><div class="table-wrap" style="max-height:420px"><table><thead><tr><th>Участник</th><th>Организация</th><th>Формат</th><th>Создан</th><th>Действие</th></tr></thead><tbody><?php foreach ($testRecords as $testRecord): ?><tr><td><strong><?= h($testRecord['full_name']) ?></strong><div class="muted"><?= h($testRecord['participant_code']) ?></div></td><td class="test-org"><?= h($testRecord['organization']) ?></td><td><?= $testRecord['participation_format']==='offline'?'Очно':'Онлайн' ?></td><td><?= h(date('d.m.Y H:i', strtotime($testRecord['created_at']))) ?></td><td><form class="test-actions" method="post"><input type="hidden" name="csrf" value="<?= h($dashboardCsrf) ?>"><input type="hidden" name="action" value="delete_test"><input type="hidden" name="participant_id" value="<?= (int)$testRecord['id'] ?>"><label><input type="checkbox" name="confirm" value="yes" required> подтверждаю удаление</label><button class="delete-btn" type="submit">Удалить тестовую запись</button></form></td></tr><?php endforeach; ?></tbody></table></div><?php endif; ?>
</section>
</main><script>
(() => { const q=document.getElementById('q'), f=document.getElementById('fmt'), s=document.getElementById('sts'), scan=document.getElementById('scan'), rows=[...document.querySelectorAll('#people tbody tr')]; function apply(){const text=(q.value||'').trim().toLowerCase(); rows.forEach(r=>{const okText=!text||r.dataset.search.includes(text); const okF=!f.value||r.dataset.format===f.value; const okS=!s.value||r.dataset.status===s.value; r.hidden=!(okText&&okF&&okS);});} [q,f,s].forEach(el=>el.addEventListener('input',apply)); if(scan){scan.focus();scan.select();} setInterval(()=>{const active=document.activeElement;if(document.hidden||(active&&active!==scan&&active.matches('input,select,textarea'))||(scan&&scan.value))return;location.reload();},60000); })();
</script></body></html>
