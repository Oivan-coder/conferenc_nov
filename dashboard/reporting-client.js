(() => {
  ...
  const organizerLabel = (value) => {
    ...
    if (n.includes('моники')) return 'МОНИКИ';
    if (n.includes('министерство здравоохранения') && (n.includes('московской области') || /\bмо\b/.test(n))) return 'Минздрав МО';
    return '';
  };
  ...
  const classifyOrg = (value) => {
    ...
    for (const marker of ['минздрава', 'фмба', 'рманпо', 'пспбгму', 'рниму', 'ростгму', 'нмиц', 'мкнц', 'рнпц', 'црб', 'гкб', 'сгб', 'бсмп', 'квд']) {
