...
'''
Path('participant.php').write_text(participant, encoding='utf-8')
...
echo $ics;
'''
Path('calendar.php').write_text(calendar, encoding='utf-8')

Path('ticket.php').write_text(r'''<?php
header('Cache-Control: private, no-store, max-age=0');
header('Pragma: no-cache');
header('X-Robots-Tag: noindex, nofollow, noarchive', true);
header('Referrer-Policy: no-referrer');
...
qr = Path('api/qr.php')
qtext = qr.read_text(encoding='utf-8')
old_qr = "$ticketUrl = 'https://rclsmo.ru/ticket.php?t=' . rawurlencode($token);"
new_qr = "$ticketUrl = 'https://rclsmo.ru/participant.php?t=' . rawurlencode($token);"
if old_qr not in qtext: raise SystemExit('qr marker missing')
qr.write_text(qtext.replace(old_qr, new_qr, 1), encoding='utf-8')

reg = Path('api/register.php')
text = reg.read_text(encoding='utf-8')
needle = "function qrImageUrl(string $qrToken): string {\n    return 'https://rclsmo.ru/api/qr.php?t=' . rawurlencode($qrToken);\n}\n"
if needle not in text: raise SystemExit('helper marker missing')
...
text = text.replace("$safeTicketUrl = htmlspecialchars(ticketUrl($qrToken), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');", "$safeTicketUrl = htmlspecialchars(participantUrl($qrToken), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');\n    $safeCalendarUrl = htmlspecialchars(calendarUrl($qrToken), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');", 1)
text = text.replace("Открыть билет с QR-кодом</a></div>'", "Открыть мой билет</a></div>'\n        . '<div style=\"text-align:center;margin:12px 0 8px;\"><a href=\"' . $safeCalendarUrl . '\" style=\"display:inline-block;color:#214f3b;text-decoration:underline;font-size:14px;font-weight:700;\">Добавить форум в календарь</a></div>'", 1)
text = text.replace("$safeLiveUrl = htmlspecialchars(liveUrl($onlineToken), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');", "$safeLiveUrl = htmlspecialchars(liveUrl($onlineToken), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');\n    $safeParticipantUrl = htmlspecialchars(participantUrl($onlineToken), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');\n    $safeCalendarUrl = htmlspecialchars(calendarUrl($onlineToken), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');", 1)
online_marker = ". '<p style=\"font-size:13px;line-height:1.6;color:#607268;margin:18px 0 0;\">Ссылка персональная. По ней система фиксирует фактическое онлайн-присутствие участника. Не пересылайте её другим людям.</p>"
if online_marker not in text: raise SystemExit('online marker missing')
text = text.replace(online_marker, ". '<div style=\"text-align:center;margin:12px 0 4px;\"><a href=\"' . $safeParticipantUrl . '\" style=\"display:inline-block;color:#214f3b;text-decoration:underline;font-size:14px;font-weight:700;margin-right:12px;\">Мой билет</a><a href=\"' . $safeCalendarUrl . '\" style=\"display:inline-block;color:#214f3b;text-decoration:underline;font-size:14px;font-weight:700;\">Добавить в календарь</a></div>'\n        . '<p style=\"font-size:13px;line-height:1.6;color:#607268;margin:18px 0 0;\">Ссылка персональная. По ней система фиксирует фактическое онлайн-присутствие участника. Не пересылайте её другим людям.</p>", 1)
resp_old = "    if ($isTestRequest && $registrationStatus === 'confirmed') {\n        if ($format === 'online') $response['live_url'] = liveUrl((string)$onlineToken);\n        else $response['ticket_url'] = ticketUrl($qrToken);\n    }"
resp_new = "    if ($registrationStatus === 'confirmed') {\n        $accessToken = $format === 'online' ? (string)$onlineToken : $qrToken;\n        $response['participant_url'] = participantUrl($accessToken);\n        $response['calendar_url'] = calendarUrl($accessToken);\n        if ($isTestRequest) {\n            if ($format === 'online') $response['live_url'] = liveUrl((string)$onlineToken);\n            else $response['ticket_url'] = participantUrl($qrToken);\n        }\n    }"
if resp_old not in text: raise SystemExit('response marker missing')
reg.write_text(text.replace(resp_old, resp_new, 1), encoding='utf-8')

js = Path('js/modules/registration-2026.js')
jtext = js.read_text(encoding='utf-8')
helper = "    function setMessage(element, text, state = '') {\n        if (!element) return;\n        element.textContent = text || '';\n        if (state) element.dataset.state = state;\n        else delete element.dataset.state;\n    }\n"
if helper not in jtext: raise SystemExit('js helper missing')
jtext = jtext.replace(helper, helper + "\n    function setMessageAction(element, text, label, url, state = 'success') {\n        setMessage(element, text, state);\n        if (!element || !url) return;\n        const br = document.createElement('br');\n        const link = document.createElement('a');\n        link.href = url;\n        link.textContent = label;\n        link.style.display = 'inline-block';\n        link.style.marginTop = '8px';\n        link.style.fontWeight = '700';\n        element.append(br, link);\n    }\n", 1)
jtext = jtext.replace("setMessage(message, `Онлайн-регистрация подтверждена. Код участника: ${code}. Персональная ссылка направлена на почту.`, 'success');", "setMessageAction(message, `Онлайн-регистрация подтверждена. Код участника: ${code}. Персональная ссылка направлена на почту.`, 'Открыть мой билет', result.participant_url, 'success');", 1)
jtext = jtext.replace("setMessage(message, `Очная регистрация подтверждена. Код участника: ${code}. QR-билет направлен на почту.`, 'success');", "setMessageAction(message, `Очная регистрация подтверждена. Код участника: ${code}. QR-билет направлен на почту.`, 'Открыть мой билет', result.participant_url, 'success');", 1)
js.write_text(jtext, encoding='utf-8')
